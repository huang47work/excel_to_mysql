...
//                int num=rs.getInt("num");
     ...
     * @return
     */
    public static List<BankEntity> getAllByExcel(String file){
        List<BankEntity> list=new ArrayList<BankEntity>();
        try {
...
//            Sheet rs=rwb.getSheet("Test Shee 1");//或者rwb.getSheet(0)
            ...
            for (int i = 1; i < rows; i++) {
                for (int j = 0; j < clos; j++) {
                    ...
                    String bankCode = rs.getCell(j++, i).getContents();

                    System.out.println("bankName:"+bankName+" bankCode:"+bankCode);
                    list.add(new BankEntity(bankName, bankCode));
                }
            }
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
            //得到表格中所有的数据
            List<BankEntity> listExcel=ImportService.getAllByExcel("e://e_banks.xls");
        /*//得到数据库表中所有的数据
        List<BankEntity> listDb=StuService.getAllByDb();*/

            DBHelper db=new DBHelper();

            for (BankEntity bankEntity : listExcel) {
//                if (!ImportService.isExist(name)) {
                    //不存在就添加
                    String sql="insert into import_test_table (bank_name,bank_code) values(?,?)";
